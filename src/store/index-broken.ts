...
try {
  ...
  console.warn(
    'Failed to create PersistenceService, using mock implementation:',
    error
  )
  // Create a minimal mock implementation for tests
  persistenceService = {
    saveProject: async () => true,
    loadProject: async () => null,
    createProject: async (name: string, description: string, canvas: any) => ({
      id: 'mock-id',
      name,
      description,
      canvas,
...
const storeCreator = (set, get) => {
  ...
  return {
    ...
    saveProject: async () => {
      const state = get()
      const currentProject = state.application.persistence.currentProject
      ...
      try {
        // Create project data with current state
        ...
        // Use persistence service (real or mock)
        const success = await persistenceService.saveProject(
          currentProject.id,
          projectData
        )
    ...
    loadProject: async (projectId: string) => {
      ...
      try {
        // Use persistence service (real or mock)
        const projectData = await persistenceService.loadProject(projectId)
        ...
        if (projectData) {
          set(
            draft => {
              ...
              draft.application.persistence.currentProject = projectData
              draft.application.persistence.isDirty = false
    ...
    createProject: async (name: string, description?: string) => {
      try {
        ...
        // Use persistence service (real or mock) to create project
        const project = await persistenceService.createProject(
          name,
          description || '',
          currentState.application.canvas,
          undefined // Default user settings
        )
        ...
        set(
          state => {
            ...
            state.application.persistence.projectList.push(metadata)
          },
          false,
          'createProject'
        )
        ...
        set(
          draft => {
            draft.error = `Failed to create project: ${error}`
          },
          false,
          'createProject:error'
        )
    ...
    deleteProject: async (projectId: string) => {
      try {
        // Use persistence service (real or mock) to delete project
        const success = await persistenceService.deleteProject(projectId)
        ...
        if (success) {
          set(
            state => {
              ...
              if (
                state.application.persistence.currentProject?.id === projectId
              ) {
                state.application.persistence.currentProject = null
              }
            },
            false,
            'deleteProject'
    ...
    duplicateProject: async (projectId: string, newName?: string) => {
      try {
        // Use persistence service (real or mock) to duplicate project
        const duplicatedProject = await persistenceService.duplicateProject(
          projectId,
          newName
        )
        ...
        set(
          state => {
            // Update project list with new duplicated project
            const metadata = {
              ...
              size: duplicatedProject.metadata?.size || 0,
              componentCount: duplicatedProject.canvas.components.size,
            }
            state.application.persistence.projectList.push(metadata)
          },
          false,
...
// Persistence service hook
export const usePersistenceService = () => persistenceService
